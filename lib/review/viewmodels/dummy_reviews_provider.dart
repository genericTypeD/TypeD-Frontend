...
// 더미 데이터 필터링을 위한 프로바이더
final dummyPrivateReviewsProvider = Provider<List<Review>>((ref) {
  final dummyReviews = ref.watch(dummyReviewsProvider);
  return dummyReviews.where((review) => !review.isPublic).toList();
});

final dummyPublicReviewsProvider = Provider<List<Review>>((ref) {
  final dummyReviews = ref.watch(dummyReviewsProvider);
  return dummyReviews.where((review) => review.isPublic).toList();
});
